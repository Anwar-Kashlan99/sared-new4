...
export default function Password() {
  ...
  const handlePasswordReset = async () => {
    try {
      const result = await forgetPassword(userDetails.data.email).unwrap();
      toast.success(t("OTP sent for password reset!"));

      if (result.status === true) {
        toast.success("OTP has been sent to your email!"); // Show success toast notification
        navigate("/verify-otp", {
          state: {
            email: userDetails.data.email,
            operationContext: "resetPassword",
          },
        });
      } else {
        ...
        >
          <Box>
            ...
            >
              ...
              >
                {t("Hello")} {userDetails.data?.username}
              </Typography>
            </Box>
            <Formik
            ...
            >
              ...
              >
                ...
                >
                  <Avatar
                    src={`${process.env.REACT_APP_SERVER_DOMAIN}/${userDetails.data?.profile}`}
                    alt="avatar"
                    style={{
                      width: "100%",
                      height: "100%",
